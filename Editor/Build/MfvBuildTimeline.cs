...
{
    /// <summary>
    /// The timeline a built world plays: a copy of the authoring timeline with the MFV
    ...
    /// </summary>
    public static class MfvBuildTimeline
    {
        public const string Folder = MfvShowSetup.GeneratedFolder + "/Generated";

        public static bool HasMfvTracks(TimelineAsset timeline)
        {
            return MfvShowCompiler.CollectTracks(timeline).Count > 0;
        }
        ...
        public static TimelineAsset Generate(TimelineAsset source, List<string> errors)
        {
            ...
            MfvShowSetup.EnsureFolder(Folder);
        ...
        private static void RemoveMfvTracks(TimelineAsset timeline, TrackAsset track)
        {
            ...
            {
                RemoveMfvTracks(timeline, child);
            }
        }

        /// <summary>
        /// Points <paramref name="director"/> at <paramref name="build"/> and carries every
        /// binding over. Tracks are matched by their position among non MFV tracks, which is
        /// stable because the copy only lost MFV tracks.
        /// </summary>
        public static void SwapAndRebind(PlayableDirector director, TimelineAsset source, TimelineAsset build)
        {
            var sourceTracks = FlattenWithoutMfv(source);
            var buildTracks = FlattenWithoutMfv(build);
        ...
        private static List<TrackAsset> FlattenWithoutMfv(TimelineAsset timeline)
        {
