    ...
    public class MfvGoboPalette : MfvPaletteStrip<MfvGoboStop>
    {
        ...
        private List<MfvGoboStop> Catalog
        {
            get
            {
                if (_catalog == null)
                {
                    ...
                    for (var index = MfvGoboLibrary.FirstPatternIndex; index <= MfvGoboLibrary.GoboCount; index++)
                    {
                        _catalog.Add(new MfvGoboStop(index));
                    }
                }
        ...
        protected override void BuildExtras()
        {
            ...
            foreach (var entry in Catalog)
            {
                ...
                tile.AddToClassList("mfv-swatch");
                tile.tooltip = MfvGoboLibrary.GetName(entry.goboIndex) + " を追加";
                tile.RegisterCallback<PointerDownEvent>(evt =>
                {
                    if (evt.button == 0)
                    {
        ...
        protected override VisualElement CreateSwatch(MfvGoboStop stop)
        {
            ...
            {
                swatch.tooltip = MfvGoboLibrary.GetName(stop.goboIndex);
                var image = new Image
                {
                    image = MfvGoboLibrary.Load(stop.goboIndex),
