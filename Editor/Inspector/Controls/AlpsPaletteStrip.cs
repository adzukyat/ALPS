...
namespace AdzukiSoft.ALPS.Editor
{
    ...
    /// </summary>
    public abstract class AlpsPaletteStrip<TStop> : VisualElement where TStop : class
    {
            ...
            Action<int> setSelected)
        {
            ...
            AddToClassList("alps-palette-strip");

            var header = new VisualElement();
            header.AddToClassList("alps-row");
            header.AddToClassList("alps-row--top");
            ...
            Editor.AddToClassList("alps-palette__editor");
            Add(Editor);

            Extras = new VisualElement();
            Extras.AddToClassList("alps-palette__extras");
            Add(Extras);
        }
        ...
        protected void RefreshSwatches()
        {
            ...
            for (var i = 0; i < _stops.Count; i++)
            {
                ...
                swatch.AddToClassList("alps-swatch");
                swatch.EnableInClassList("alps-swatch--selected", i == selected);
        ...
        private VisualElement CreateAddTile()
        {
            ...
            tile.EnableInClassList("alps-swatch--active", AddActive);
        ...
        private VisualElement CreateDeleteTile()
        {
            ...
            tile.AddToClassList("alps-swatch");
            tile.AddToClassList("alps-swatch--action");
            tile.EnableInClassList("alps-swatch--disabled", !CanDelete);
            ...
            icon.SetPaths(AlpsIcons.Trash);
            icon.AddToClassList("alps-swatch__icon");
            tile.Add(icon);
        ...
        private void RegisterGestures(VisualElement swatch, int index)
        {
            ...
            swatch.RegisterCallback<PointerDownEvent>(evt =>
            {
                ...
                {
                    other.EnableInClassList("alps-swatch--selected", other == swatch);
                }
            ...
            swatch.RegisterCallback<PointerMoveEvent>(evt =>
            {
                ...
                if (!_dragging)
                {
                    ...
                    _dragging = true;
                    swatch.AddToClassList("alps-swatch--dragging");
                }
        ...
        {
            _pressed?.RemoveFromClassList("alps-swatch--dragging");
            _pressed = null;
            _pressedIndex = -1;
            _dragging = false;
        }
    ...
    public class AlpsColorPalette : AlpsPaletteStrip<AlpsColorStop>
    {
        ...
        public AlpsColorPalette(
            string label,
            List<AlpsColorStop> stops,
            Func<int> getSelected,
            Action<int> setSelected)
        ...
        protected override VisualElement CreateSwatch(AlpsColorStop stop)
        {
            var swatch = new VisualElement { tooltip = stop.isGradient ? GradientLabel : SolidLabel };
            if (stop.isGradient)
            {
                var preview = AlpsGradientPreview.Build(stop.gradient);
        ...
        protected override AlpsColorStop CloneStop(AlpsColorStop stop)
        {
            return new AlpsColorStop(stop);
        }

        protected override AlpsColorStop CreateDefaultStop()
        {
        ...
        protected override void BuildEditor(AlpsColorStop stop)
        {
            ...
            if (stop.isGradient)
            {
                ...
                field.AddToClassList("alps-palette__value");
    ...
    /// <summary>
    /// Gobo palette, where the OFF tile is VRSL's open beam. + opens a picker of OFF and
    /// every patterned gobo VRSL ships with. It stays open so a sequence can be built by
    /// clicking tiles one after another.
    /// </summary>
    public class AlpsGoboPalette : AlpsPaletteStrip<AlpsGoboStop>
    {
        private readonly Func<bool> _getPickerOpen;
        private readonly Action<bool> _setPickerOpen;
        private List<AlpsGoboStop> _catalog;

        public AlpsGoboPalette(
            string label,
            List<AlpsGoboStop> stops,
            Func<int> getSelected,
            Action<int> setSelected,
        ...
        /// <summary>OFF followed by the patterned gobos, built once per palette.</summary>
        private List<AlpsGoboStop> Catalog
        {
            get
            {
                if (_catalog == null)
                {
                    _catalog = new List<AlpsGoboStop> { new AlpsGoboStop() };
                    for (var index = AlpsGoboLibrary.FirstPatternIndex; index <= AlpsGoboLibrary.GoboCount; index++)
                    {
                        _catalog.Add(new AlpsGoboStop(index));
                    }
                }
        ...
        protected override void BuildExtras()
        {
            ...
            row.AddToClassList("alps-row--top");
            row.Add(CreateEditorGutter());

            var picker = new VisualElement();
            picker.AddToClassList("alps-palette");
            picker.AddToClassList("alps-gobo-picker");
            foreach (var entry in Catalog)
            {
                ...
                tile.RegisterCallback<PointerDownEvent>(evt =>
                {
                    if (evt.button == 0)
                    {
                        AddStop(new AlpsGoboStop(entry));
                        evt.StopPropagation();
                    }
                });
                picker.Add(tile);
        ...
        public IReadOnlyList<AlpsGoboStop> CatalogStops => Catalog;

        protected override VisualElement CreateSwatch(AlpsGoboStop stop)
        {
            var swatch = new VisualElement();
            swatch.AddToClassList("alps-swatch--gobo");

            if (stop.IsOff)
            {
                swatch.AddToClassList("alps-swatch--off");
                swatch.Add(new Label("OFF") { pickingMode = PickingMode.Ignore });
            }
            else
            {
                swatch.tooltip = AlpsGoboLibrary.GetName(stop.goboIndex);
                var image = new Image
                {
                    image = AlpsGoboLibrary.Load(stop.goboIndex),
                    scaleMode = ScaleMode.ScaleToFit,
                    pickingMode = PickingMode.Ignore,
                };
                image.AddToClassList("alps-swatch__image");
                swatch.Add(image);
            }
        ...
        protected override AlpsGoboStop CloneStop(AlpsGoboStop stop)
        {
            return new AlpsGoboStop(stop);
        }

        protected override AlpsGoboStop CreateDefaultStop()
        {
            return new AlpsGoboStop();
        }
    }

    /// <summary>Renders a Gradient into a small texture so a swatch can show it.</summary>
    internal static class AlpsGradientPreview
    {
