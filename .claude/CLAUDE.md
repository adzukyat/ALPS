...
- A phase is a wave or random. A wave cycle is four shares, rise, high hold, fall and the low hold they leave over (`AlpsShowEvaluator.Wave`), and the one ease shapes the rise and runs backwards on the fall. The return leg (`IsReturnLeg`, blackout on return) is the fall plus the low hold.
...
`AlpsArrangement` (`Runtime/Fixtures`) lays its direct children out in hierarchy order, inactive ones included, on a line, circle or arc, polygon, rectangle or grid, from an `AlpsArrangementSettings` whose sizes, offsets and rotations are `AlpsAnimatableValue`s. It is editor only for now: nothing moves it over time, so its rows offer S and not R, and a range saved on a value is ignored. The geometry is `AlpsArrangementEvaluator` (`Runtime/Playback`), statics over a layout row and one resolved value per `Value*` index, kept inside what UdonSharp compiles so a Timeline driven arrangement can run it in Udon later. Values are resolved in the model with the same math as `ResolveScalar` without a range, and a test holds the two together.

`Editor/Arrangement` holds the rest. `AlpsArrangementLayout` writes only children whose pose differs, which keeps prefab instances free of empty overrides and is what stops the layout from looping. `AlpsArrangementWatcher` lays arrangements out again from `ObjectChangeEvents` (children added, removed, reordered, reparented or moved, fixtures added below, the component added or reset). It records into the current undo group, never increments it, skips the publish that follows an undo or redo, and waits while a handle is held. The first layout (`Initialize`) reorders the children to keep an existing fixture group's order and starts the line on the first and last child. While an arrangement is enabled, the fixture group on the same object follows the children's order and `AlpsFixtureGroupEditor` hides its list.
