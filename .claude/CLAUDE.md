...
Playback cost matters more than anything else here, since every array read and float operation is an Udon extern. The compiler precomputes what does not change over time: a positions table (`ClipPositionStart`, the order position and mirror flag of every group member, shared by clips with the same group, order and grouping) and a time index (`bucketStart` / `bucketClips`, the clips overlapping each time bucket in clip order). Each frame `ActiveClips` weighs only the current bucket's clips once, and `EvaluateFixture` walks that list for every fixture. A clip alone on its layer at full weight, the usual case, writes straight into the frame without blending. `EvaluateClip` works out the clip's shared phase once (`ClipUsesPhase`, set by the compiler, says whether anything reads it) and every value without its own phase or a phase offset reuses it.

`AlpsPlaybackBenchmark` (explicit, enters play mode) times the compiled Udon player on 48 VRSL movers and breaks the cost down per effect. Run it by name with `-testFilter AdzukiSoft.ALPS.Tests.AlpsPlaybackBenchmark`, it writes `TestResults~/benchmark.txt`.
...
There is no manual bake. `Editor/Build/AlpsBuildHooks.cs` validates and generates build Timelines before a VRChat build (`IVRCSDKBuildRequestedCallback`), and `IProcessSceneWithReport` (order -100, before UdonSharp strips proxies) runs `AlpsShowApplier.Apply` on the build scene copy and on entering play mode. Apply creates an `ALPS Show Player` under each show director on the scene copy (`AlpsShowSetup.GetOrCreatePlayer`, reusing one an older version left in the scene), compiles the show into it, swaps the director to a generated Timeline without ALPS tracks with other bindings carried over, and strips every `AlpsTarget` (containers and fixtures). Directors sharing a Timeline share one build copy. `AlpsBuildReferences` then points every other serialized reference to an authoring Timeline (component fields, UnityEvent arguments, UdonBehaviour public variables) at its build copy, and gives any director holding bindings for the authoring tracks the same bindings on the build tracks. The Udon program asset is made by the build preflight, and before play mode by `AlpsPlayModePreflight`. The strip runs on every scene, with a show or without one. The authoring scene and Timeline are never modified.

### GPU playback (experimental)

Switched per machine with `ALPS > Experimental > GPU Playback` (`AlpsGpuPlayback`, an EditorPref that also sets `AlpsPreviewDriver.UseGpu`). It applies to the preview, play mode and builds. The show is evaluated by a shader and handed to VRSL as its DMX grid, so Udon only sets the time and calls `VRCGraphics.Blit` three times a frame.

- `AlpsShowPlayer.PackShowData` lays the compiled arrays, the fixture defaults and per fixture DMX info out as one RFloat texture of 1024 wide rows with a header of section offsets. `Runtime/Gpu/AlpsEvaluator.hlsl` mirrors those offsets and every evaluator constant, so a layout change touches both.
- `Hidden/ALPS/Frames` evaluates every fixture into a 4 texel wide RGBA row holding the 13 frame channels. `Hidden/ALPS/DMX Grid` pass 0 lays them out as VRSL's 26 x 240 horizontal grid (bound to `_Udon_DMXGridRenderTexture`, `...Movement` and `...StrobeOutput`), pass 1 writes the gobo angle as the phase VRSL's spin timer grid holds (`_Udon_DMXGridSpinTimer`), which is why no gobo patch is needed there.
- Fixture n goes on absolute DMX channel 13n + 1 (`ConfigureVRSLDmx`, at most `GpuMaxFixtures` = 118). VRSL reads channel offset o of fixture n from texel (2o + 1, 2n + 1): its `IndustryRead` takes the row as an int, which drops the fraction of channel / 13. `AlpsGpuEvaluatorTests.Grid_ReadsBackThroughVrslsOwnFunctions` checks this through VRSL's own `getValueAtCoords` (the `Hidden/ALPS/Tests/VRSL Probe` shader), and `Diagnose_WhereVrslReads` dumps the texel of every channel.
- The dimmer stays at 1 and the colour carries brightness times `_AlpsBrightnessGain` (2), which keeps brightness linear on every VRSL mesh and lets it pass 100%. Pan and tilt ride on base rotations of 0 and 90.
- Not covered yet: cone length (no DMX channel, it keeps the fixture default), tracking a user, strobe, and more than one GPU show per scene (the grid textures are global). Random phases and flicker use a noise of the GPU's own, so they do not match the C# evaluator.
- The shared materials and targets are assets under `Assets/ALPS/Gpu`, made by the preflights. The GPU tests need a graphics device and are ignored under `-nographics`, so run them without it. `AlpsPlaybackBenchmark.Capture_Cpu` / `Capture_Gpu` render the same moment on both paths into `TestResults~` to compare by eye.

### Fixtures

`AlpsFixture` is the adapter seam (`AdapterKind`, capture default, apply frame, preview property gathering). Only `AlpsVRSLFixture` exists. The Udon side cannot call the MonoBehaviour adapters, so the VRSL mapping lives as statics on `AlpsShowPlayer` (`ApplyVRSL`, `CaptureVRSL`) and the fixture delegates to them. Gobo rotation is written into each renderer's MaterialPropertyBlock after VRSL rebuilds its own block. The editor preview always goes through VRSL (`ApplyVRSL`). The player does so only the first time and for one fixture per frame in turn, which also repairs a block VRSL rebuilt on its own. Otherwise it compares the frame with the last one written (`FrameChange`, a channel mask) and writes the shader properties of the changed channels straight into the renderers' blocks (`WriteChanges`), with the same conversions (`Vrsl*` helpers) VRSL 2.8.4 applies in `_UpdateInstancedProperties`, so VRSL's fields go stale in between. Level 4 therefore compares property blocks, not fields, after the first frame.
