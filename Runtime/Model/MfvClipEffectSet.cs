...
namespace ManeuverForVRC
{
    ...
    public class MfvClipEffectSet
    {
        ...
        public MfvClipEffectSet() { }

        /// <summary>Deep copy, used to load a clip from a profile and to save it back.</summary>
        public MfvClipEffectSet(MfvClipEffectSet other)
        {
            order = other.order;
            phaseExpanded = other.phaseExpanded;
            phase = new MfvPhaseSettings(other.phase);
            effects = new List<MfvEffect>();
            foreach (var effect in other.effects)
            {
                effects.Add(new MfvEffect(effect));
            }
