...
namespace ManeuverForVRC.Tests
{
    ...
    /// </summary>
    public class MfvLayoutAuditTests
    {
        /// <summary>
        /// The 476px minimum card width plus two wider
        /// inspectors. Narrower hosts scroll (see the min-width on .mfv-root).
        /// </summary>
        private static readonly float[] Widths = { 476f, 720f, 1000f };

        private static MfvClipEffectSet BuildFixtureSet()
        {
            return MfvInspectorFixture.Build();
        }
        ...
        private static IEnumerator Measure(float width, System.Action<VisualElement> assert)
        {
            ...
            var view = new MfvClipInspectorView(BuildFixtureSet());
        ...
        public IEnumerator DesignTokens_SurviveTheUssRoundTrip()
        {
            ...
            var sizes = new (string ClassName, float Width, float Height)[]
            {
                ...
                ("mfv-numberbox", 70f, 23f),
                ("mfv-rangeflag__box", 24f, 21f),
                ("mfv-switch__track", 36f, 20f),
            ...
            yield return Measure(476f, view =>
            {
                ...
                foreach (var tile in Visible(view).Where(e => e.ClassListContains("mfv-easegrid__tile")))
                {
                    var compact = tile.GetFirstAncestorOfType<MfvPhaseSettingsView>()
                        ?.ClassListContains("mfv-nested") == true;
                ...
                foreach (var graph in Visible(view).Where(e => e.ClassListContains("mfv-graph")))
                {
                    var compact = graph.ClassListContains("mfv-graph--compact");
                ...
                foreach (var body in Visible(view).Where(e => e.ClassListContains("mfv-card__body")))
                {
                ...
                foreach (var header in Visible(view).Where(e => e.ClassListContains("mfv-card__header")))
                {
                    Assert.AreEqual(
        ...
        {
            // MfvNumberBox is a BaseField, so an unscoped margin rule loses to the
            // `.mfv-root .unity-base-field` reset and the box ends up flush against the
            // track, which then draws the min thumb inside the box.
            yield return Measure(width, view =>
            {
                var wrong = new List<string>();

                foreach (var box in Visible(view).Where(e => e.ClassListContains("mfv-numberbox--left")))
                {
                ...
                foreach (var box in Visible(view).Where(e => e.ClassListContains("mfv-numberbox--right")))
                {
        ...
        public IEnumerator SliderThumbsStayClearOfTheirValueBoxes([ValueSource(nameof(Widths))] float width)
        {
            ...
            yield return Measure(width, view =>
            {
                var boxes = Visible(view)
                    .Where(e => e.ClassListContains("mfv-numberbox"))
                    .Where(e => e.layout.width > 1f)
                    .ToList();

                var collisions = new List<string>();

                foreach (var thumb in Visible(view).Where(e => e.ClassListContains("mfv-slider__thumb")))
                {
